  ...
  const result = await Tesseract.recognize(canvas, 'eng', {
    logger: m=>{
      if(m.status === 'recognizing text'){
        setCameraStatus(`Reading label ${Math.round((m.progress || 0) * 100)}%`);
      }
    }
  });
