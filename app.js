...
function searchRows(q){ const s=(q||"").trim().toLowerCase(); if(!s) return []; const isId=/^[a-z0-9]+(\/?[a-z0-9]+)?$/.test(s); return state.rowIndex.filter(r=> isId ? r.combined_id.toLowerCase().includes(s) : r.material_name.toLowerCase().includes(s)).slice(0,100); }
...
function extractSlabIds(text){
  const clean = normalizeScanText(text);
  const compactSlashes = clean.replace(/\s*\/\s*/g,'/');
  const patterns = [
    /(?:^|[^A-Z0-9])([A-Z]?\d{4,6}\/[A-Z]\d{3,5})(?=$|[^A-Z0-9])/g,
    /(?:^|[^A-Z0-9])(\d{4,6}\/\d{3,5})(?=$|[^A-Z0-9])/g
  ];
  const ids = [];
  patterns.forEach(pattern=>{
    for(const match of compactSlashes.matchAll(pattern)){
      const id = match[1];
...
function findSlabByScannedId(id){
  const target = (id||'').toUpperCase();
  return state.rowIndex.find(r=>(r.combined_id||'').toUpperCase()===target)
    || state.rowIndex.find(r=>(r.combined_id||'').toUpperCase().includes(target) || target.includes((r.combined_id||'').toUpperCase()));
}
...
function openNumberSearch(value){
  const search = document.getElementById('search');
  search.value = value || '';
  renderSearchResults(searchRows(search.value));
  search.focus();
  search.select();
}
function showFoundSlab(row, scannedId){
  state.scanCandidate = row;
  showScanResult(`<strong>Found slab ${row.combined_id || scannedId}</strong><span>${row.material_name || 'Material unknown'} • ${row.size_mm || '?'} • ${row.thickness_mm || '?'}mm</span><div class="result-actions"><button class="primary" id="confirmScanBtn" type="button">Confirm Available</button><button class="ghost" id="searchScannedBtn" type="button">Search instead</button></div>`);
  document.getElementById('confirmScanBtn')?.addEventListener('click', ()=>{
    markAvailable(row.key);
    showScanResult(`<strong>Marked Available</strong><span>${row.combined_id} • ${row.material_name || ''}</span>`);
    setCameraStatus('Scan complete. You can scan another label or close the camera.');
  });
  document.getElementById('searchScannedBtn')?.addEventListener('click', ()=>openNumberSearch(scannedId || row.combined_id));
}
function showScanFallback(ids, rawText){
  const debug = rawText ? `<div class="scan-debug">Read text:\n${escapeHtml(rawText.slice(0, 600))}</div>` : '';
  if(ids && ids.length){
    const id = ids[0];
    showScanResult(`<strong>Found ${id}, but it is not in the slab list.</strong><span>Search the slab by number, or enter it manually if it is new.</span><div class="result-actions"><button class="primary" id="searchUnknownBtn" type="button">Search number</button><button class="ghost" id="manualUnknownBtn" type="button">Add manually</button></div>${debug}`);
    document.getElementById('searchUnknownBtn')?.addEventListener('click', ()=>openNumberSearch(id));
  }else{
    const message = rawText ? 'I read some text, but not a slab number.' : 'The camera did not read usable label text.';
    showScanResult(`<strong>Couldn’t detect a slab number.</strong><span>${message} Please search by number, or add it manually if the label only shows material/size.</span><div class="result-actions"><button class="primary" id="focusSearchBtn" type="button">Search by number</button><button class="ghost" id="manualFallbackBtn" type="button">Add manually</button></div>${debug}`);
    document.getElementById('focusSearchBtn')?.addEventListener('click', ()=>openNumberSearch(''));
  }
  document.getElementById('manualUnknownBtn')?.addEventListener('click', openAddModal);
  document.getElementById('manualFallbackBtn')?.addEventListener('click', openAddModal);
}
async function setupBarcodeDetector(){
  ...
  state.barcodeDetector = new BarcodeDetector({ formats });
...
function makeWorkingCanvas(source, rotation){
...
function makeCenterCropCanvas(source){
  const canvas = document.createElement('canvas');
  const cropWidth = Math.round(source.width * 0.68);
  const cropHeight = Math.round(source.height * 0.84);
  const sx = Math.round((source.width - cropWidth) / 2);
  const sy = Math.round((source.height - cropHeight) / 2);
  canvas.width = cropWidth;
  canvas.height = cropHeight;
  canvas.getContext('2d', { willReadFrequently:true }).drawImage(source, sx, sy, cropWidth, cropHeight, 0, 0, cropWidth, cropHeight);
  return canvas;
}
async function readBarcode(canvas){
  const detector = await setupBarcodeDetector();
  if(!detector) return [];
  try{
    const variants = [canvas, makeWorkingCanvas(canvas, 90), makeWorkingCanvas(canvas, -90), makeCenterCropCanvas(canvas)];
    const values = [];
    for(const variant of variants){
      const codes = await detector.detect(variant);
      codes.map(code=>code.rawValue).filter(Boolean).forEach(value=>{ if(!values.includes(value)) values.push(value); });
...
async function readText(canvas){
  ...
  const variants = [
    { label:'upright', canvas },
    { label:'right turn', canvas:makeWorkingCanvas(canvas, 90) },
    { label:'left turn', canvas:makeWorkingCanvas(canvas, -90) },
    { label:'center crop', canvas:makeCenterCropCanvas(canvas) }
  ];
  const texts = [];
  for(const variant of variants){
    setCameraStatus(`Reading printed text (${variant.label})...`);
    const result = await Tesseract.recognize(variant.canvas, 'eng', {
      logger: m=>{
        if(m.status === 'recognizing text'){
          setCameraStatus(`Reading ${variant.label} ${Math.round((m.progress || 0) * 100)}%`);
        }
      }
    });
    const text = result?.data?.text || '';
    if(text.trim()) texts.push(text);
    if(extractSlabIds(text).length){
      return texts.join('\n');
    }
  }
  return texts.join('\n');
...
async function startCamera(){
  ...
  try{
    ...
    await video.play();
    setCameraStatus('Hold the phone vertically and place one label inside the guide.');
  }catch(e){
    showScanFallback([], '');
    setCameraStatus('Camera could not open. Use number search or manual entry.');
  }
}
function stopCamera(){
...
async function scanCurrentFrame(){
  ...
  try{
    const barcodeTexts = await readBarcode(canvas);
    let rawText = barcodeTexts.join('\n');
    let ids = barcodeTexts.flatMap(extractSlabIds);
    if(!ids.length){
      setCameraStatus('No barcode match. Reading printed text...');
      rawText = await readText(canvas);
      ids = extractSlabIds(rawText);
    }
    const row = ids.map(findSlabByScannedId).find(Boolean);
    if(row){
      showFoundSlab(row, ids[0]);
      setCameraStatus('Check the result and confirm if it is correct.');
    }else{
      showScanFallback(ids, rawText);
      setCameraStatus(ids.length ? 'Number found, but not in the list.' : 'No slab number detected.');
    }
  }catch(e){
    showScanFallback([], '');
    setCameraStatus('Could not read the label. Use number search or manual entry.');
  }finally{
...
async function init(){
  ...
  document.getElementById('captureBtn').addEventListener('click', scanCurrentFrame);
  document.getElementById('retryScanBtn').addEventListener('click', scanCurrentFrame);
